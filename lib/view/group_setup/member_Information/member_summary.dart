...
Akiba Hiari: ${formatCurrency((_akibaHiariTotal - totalAkiba), Provider.of<CurrencyProvider>(context, listen: false).currencyCode)}
...
""";
  ...
  Future<void> _fetchUserSpecificData(int userId) async {
    ...
    setState(() {
      _salioTotal =
          _akibaLazimaTotal + _akibaHiariTotal + _mfukoJamiiTotal - totalAkiba;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        ...
        showBackArrow: true,
        icon:
            _isMzungukoPending && isPasswordSetupComplete ? Icons.delete : null,
        onIconPressed: () {
          showDialog(
            context: context,
                ...
                child: Column(
                  children: [
                    ...
                    _buildFundSection(
                      ...
                      amount: formatCurrency(_akibaLazimaTotal, Provider.of<CurrencyProvider>(context).currencyCode),
                    ),
                    Divider(thickness: 1, color: Colors.grey[300]),
                    _buildFundSection(
                      title: AppLocalizations.of(context)!.totalSavings,
                      amount: formatCurrency((_akibaHiariTotal - totalAkiba), Provider.of<CurrencyProvider>(context).currencyCode),
                    ),
                    Divider(thickness: 1, color: Colors.grey[300]),
                    _buildFundSection(
