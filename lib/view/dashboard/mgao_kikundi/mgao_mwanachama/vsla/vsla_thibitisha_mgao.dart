...
class VslaThibishaMgao extends StatefulWidget {
  ...
  @override
  State<VslaThibishaMgao> createState() => _VslaThibishaMgaoState();
}

class _VslaThibishaMgaoState extends State<VslaThibishaMgao> {
  // Replace akiba variables with share variables
  ...
  // New method to fetch share value from katiba
  Future<double> _fetchShareValue() async {
    try {
      ...
      // Fallback to share_amount if share_value is not found
      final shareAmountData = await katibaModel
          .where('katiba_key', '=', 'share_amount')
          .where('mzungukoId', '=', widget.mzungukoId)
          .findOne();

      if (shareAmountData != null && shareAmountData is KatibaModel) {
        final value = shareAmountData.value;
        if (value != null && value.isNotEmpty) {
          return double.tryParse(value) ?? 0.0;
        }
      }
  ...
  // New method to fetch total shares for the user
  Future<void> _fetchTotalSharesForUser() async {
    try {
      ...
      print(
          'Total shares for user ${widget.user['id']}: $_totalSharesCount worth $_totalSharesValue');
    } catch (e) {
      print('Error fetching total shares for user ${widget.user['id']}: $e');
      setState(() {
        _totalSharesCount = 0;
        _totalSharesValue = 0;
      });
  ...
  Future<void> _saveMgaoData() async {
    ...
    if (existingEntry != null) {
      ...
      await userMgaoModel.where('id', '=', userMgao.id).update({
        'mgaoAmount': _totalSharesValue,
        'akibaBinafsi': 0, // No longer using akiba binafsi
        'status': 'paid',
        'type': 'personal',
        // 'shares_count': _totalSharesCount,
...
Idadi ya hisa: ${_totalSharesCount}
Thamani ya hisa: ${formatCurrency(_shareValue.toInt(), Provider.of<CurrencyProvider>(context, listen: false).currencyCode)}
Jumla ya thamani ya hisa: ${formatCurrency(_totalSharesValue.toInt(), Provider.of<CurrencyProvider>(context, listen: false).currencyCode)}
...
""";
  ...
  Future<void> deleteShares() async {
    try {
      final memberShareModel = MemberShareModel();
      final deletedRows = await memberShareModel
          .where('user_id', '=', widget.user['id'])
          .where('mzungukoId', '=', widget.mzungukoId)
          .delete();

      print('Deleted $deletedRows share records for user ${widget.user['id']}');

      if (deletedRows > 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Rekodi za hisa zimefutwa kikamilifu.')),
        );
      }
    } catch (e) {
      print('Error deleting shares for user ${widget.user['id']}: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Imeshindikana kufuta rekodi za hisa.')),
      );
    }
  }
  ...
  faini() async {
      ...
      for (var record in userFainiRecords) {
        ...
        // Add unpaid amount to paid amount
        final existingPaidFaini = userFaini.paidfaini ?? 0;
        userFaini.paidfaini = existingPaidFaini + existingUnpaidFaini;
      ...
      // Show snackbar only once after all records are processed
      if (userFainiRecords.isNotEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Malipo ya faini yamefanywa kikamilifu.')),
        );
      }
    }
    ...
    mkopo() async {
      ...
      if (results.isEmpty) {
        print("No unpaid loans found for this user.");
        return; // Return instead of throwing exception
      }

      for (var result in results) {
        RejeshaMkopoModel loanToUpdate = result as RejeshaMkopoModel;
        double unpaidLoanAmount = loanToUpdate.unpaidAmount ?? 0.0;

        if (unpaidLoanAmount > 0) {
          // Add unpaid amount to paid amount
          loanToUpdate.paidAmount = (loanToUpdate.paidAmount ?? 0.0) + unpaidLoanAmount;
          ...
          await existingLoan.where('id', '=', loanToUpdate.id).update({
            ...
            'unpaidAmount': 0.0,
            'payments': json.encode(updatedPayments),
          });

          print('Updated loan: ID = ${loanToUpdate.id}, Paid Amount = ${loanToUpdate.paidAmount}, Unpaid Amount = 0.0');
        }
      }
      ...
      if (results.isNotEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Mikopo yote imefanikiwa kulipwa!")),
        );
      }
    }
  ...
  void initState() {
    super.initState();
    _fetchTotalSharesForUser().then((_) {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        buildSummaryRow(
                          l10n.totalSharesCount,
                          _totalSharesCount.toString(),
                          color: Colors.green,
                        ),
                        buildSummaryRow(
                          l10n.shareValue,
                          _shareValue.toStringAsFixed(0),
                          color: Colors.green,
                        ),
                        buildSummaryRow(
                          l10n.totalSharesValue,
                          _totalSharesValue.toStringAsFixed(0),
                          color: Colors.green,
                        ),
                        Divider(),
                        if (_totalMgao < 0)
