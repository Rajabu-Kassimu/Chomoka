...
import 'package:chomoka/view/dashboard/meetings/new_meeting/vsla/other_fund/different_amount.dart';
import 'package:chomoka/view/dashboard/meetings/new_meeting/vsla/other_fund/contribute_same_amount.dart';
...
class _VslaMeetingDashboardState extends State<VslaMeetingDashboard> {
  ...
  void _navigateToMfukoPage(BuildContext context, MifukoMingineModel mfuko) {
    if (mfuko.utoajiType == 'Kiwango sawa') {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ContributeSameAmount(
            meetingId: widget.meetingId,
            mzungukoId: mzungukoId,
            mfukoName: mfuko.mfukoName ?? '',
            mfukoId: mfuko.id,
            contributionAmount: double.tryParse(mfuko.mfukoAmount ?? '0'),
          ),
        ),
      );
    } else {
  ...
  Widget build(BuildContext context) {
    // Calculate if all mifuko are complete
    bool allMifukoComplete = _areAllMifukoComplete();
    final l10n = AppLocalizations.of(context)!;

    return Scaffold(
      appBar: CustomAppBar(
        title: l10n.meetingTitle(meetingNumber ?? 0),
        showBackArrow: false,
      ...
      body: Container(
        child: ListView(
          children: [
            ...
            // Dynamic ListTiles for Mifuko Mingine - MOVED BEFORE NUNUA HISA
            ..._mifukoMingineList.asMap().entries.map((entry) {
              int index = entry.key;
              MifukoMingineModel mfuko = entry.value;

              // Check if previous mfuko is complete or if it's the first mfuko
              bool isPreviousComplete = index == 0
                  ? true
                  : _getMfukoStatus(_mifukoMingineList[index - 1].id) ==
                      'completed';

              // Should be active only if previous steps and previous mfuko are complete
              bool shouldBeActive = isAttendanceComplete &&
                  isChangiaMfukoJamiiComplete &&
                  isPreviousComplete;

              return ListTiles(
                tileColor:
                    _getMfukoStatus(mfuko.id) == 'completed' || !shouldBeActive
                        ? Colors.white
                        : const Color.fromARGB(255, 243, 188, 7),
                icon: Icon(Icons.account_balance_wallet,
                    color: Color.fromARGB(144, 12, 36, 252)),
                title: l10n.contributeOtherFund(mfuko.mfukoName ?? ''),
            ...
            // Nunua Hisa moved after Mifuko Mingine
            ListTiles(
              tileColor: isAkibaLazimaComplete ||
                      !isChangiaMfukoJamiiComplete ||
                      !_areAllMifukoComplete()
                  ? Colors.white
                  : const Color.fromARGB(255, 243, 188, 7),
              icon: Icon(Icons.check_box,
                  color: Color.fromARGB(144, 12, 36, 252)),
              title: l10n.buyShares,
              mark: isAkibaLazimaComplete ? 'completed' : 'pending',
              onTap: () {
                if (isAttendanceComplete &&
                    isChangiaMfukoJamiiComplete &&
                    _areAllMifukoComplete()) _navigateToSharePage(context);
              },
            ),
            ...
            ListTiles(
              ...
              title: l10n.withdrawFromMfukoJamii,
